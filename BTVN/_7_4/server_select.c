...
int32_t parse_register(char *buffer, char *id_out, char *name_out) {
    ...
    if (sscanf(buffer, "%[^:]: %s", id, name) == 2) {
        strncpy(id_out, id, MAX_ID_NAME - 1); // lưu client_id
...
void handle_client_data(client_t *clients, int32_t idx, fd_set *read_fd, uint16_t *num_clients) {
    ...
    if (!clients[idx].registered) {
        char id[MAX_ID_NAME];
        char name[MAX_ID_NAME];
        if (parse_register(buffer, id, name)) {
            strncpy(clients[idx].id, id, MAX_ID_NAME - 1);
            ...
            send(fd, msg, strlen(msg), 0);
            printf("Client id=%s dang ky ten: %s\n", id, name);
        } else {
        ...
        printf("%s %s %s\n", time_str, clients[idx].id, buffer);
